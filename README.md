...
3. 项目根目录没有 `AGENTS.md` 时，根据项目事实和 `AGENTS.template.md` 创建；已经存在时只合并缺失的通用规则，不覆盖项目原有内容。不要要求用户复制、改名或理解这些文件。
4. 根据根 `SKILL.md` 的任务表，只读取当前需要的细分技能和离线文档。
5. 检查项目已有工具和插件。只有当前任务确实需要时才推荐安装，先说明解决什么问题、版本/许可证/冲突和不安装的影响；未经用户授权不安装外部工具。
6. 向用户询问无法从项目发现且会改变产品方向的选择；不要让用户补技术术语、重复执行自动验证或管理本开发包内部文件。
...
- `AGENTS.template.md` 由 AI 用于创建或补充项目自己的长期规则，用户不需要操作它。
...
之后由 AI 读取 README、技能和文档，检查或建立项目规则，并询问真正缺失的产品信息。用户不需要知道 `SKILL.md`、`AGENTS.md` 的名字，也不需要为不同 AI 适配文件结构。
